...
namespace Core.DAL
{
    public class RamMetricsRepository : IRepository<RamMetric>
    {
        ...
        public IList<RamMetric> GetByTime(double timeFrom, double timeTo)
        {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                List<RamMetric> ramMetrics = connection.Query<RamMetric>("SELECT id, time, value FROM rammetrics WHERE time>@timeFrom AND time<@timeTo", new { timeFrom = timeFrom, timeTo =timeTo}).ToList();
                return ramMetrics;
            }
        ...
        public TimeSpan GetLastTimeSpan()
        {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                if (connection.QueryFirst<bool>("SELECT EXISTS(Select* FROM rammetrics)"))
                {
                    int maxTime = connection.QueryFirst<int>("Select Time FROM rammetrics WHERE \"Time\" = (SELECT MAX(\"Time\") FROM rammetrics)");
