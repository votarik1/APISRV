...
namespace Core.DAL
{
    public class HddMetricsRepository : IRepository<HddMetric>
    {
        ...
        public IList<HddMetric> GetByTime(double timeFrom, double timeTo)
        {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                List<HddMetric> hddNetMetrics = connection.Query<HddMetric>("SELECT id, time, value FROM hddmetrics WHERE time>@timeFrom AND time<@timeTo", new { timeFrom = timeFrom, timeTo = timeTo }).ToList();
        ...
        public TimeSpan GetLastTimeSpan()
        {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                if (connection.QueryFirst<bool>("SELECT EXISTS(Select* FROM hddmetrics)"))
                {
                    int maxTime = connection.QueryFirst<int>("Select Time FROM hddmetrics WHERE \"Time\" = (SELECT MAX(\"Time\") FROM hddmetrics)");
